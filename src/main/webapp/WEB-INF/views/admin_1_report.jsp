  ...
  <div class="content">
    ...
    <table>
      ...
      <c:choose>
        <c:when test="${not empty reports}">
          <c:forEach var="report" items="${reports}">
            <tr>
              ...
              <td>
                <c:choose>
                  ...
                  <c:when test="${report.status == 'RESOLVED'}">
                    <span class="status-resolved">처리완료</span>
                  </c:when>
              ...
              <td>${fn:substring(report.createdAt, 0, 10)}</td>
              <td>
                <form action="${pageContext.request.contextPath}/api/admin/report/handle/${report.reportId}" method="post" style="display:inline;">
                  <button type="submit" class="action-button">처리</button>
                </form>
              </td>
            </tr>
          </c:forEach>
        ...
        <c:otherwise>
          <tr>
            <td colspan="5" class="empty-message">신고 데이터가 없습니다.</td>
          </tr>
        </c:otherwise>
      </c:choose>
      </tbody>
